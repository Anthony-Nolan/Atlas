...
namespace Atlas.MatchingAlgorithm.Data.Repositories.DonorUpdates
{
    public abstract class DonorUpdateRepositoryBase : Repository
    {
            ...
            LongStopwatchCollection timerCollection = null)
        {
            ...
            using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_Overall_TimerKey))
            {
            ...
            LongStopwatchCollection timerCollection = null)
        {
            using (var transactionScope = new OptionalAsyncTransactionScope(runAllHlaInsertionsInASingleTransactionScope))
            {
                ...
                foreach (var locus in LocusSettings.MatchingOnlyLoci)
                {
                    ...
                    {
                        var insertSetupOperationTimer = timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_Overall_TimerKey);
                        ...
                        {
                            using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BlockingWait_TimerKey))
                            {
                                await upsertTask;
                            }
                        }
            ...
            LongStopwatchCollection timerCollection = null)
        {
            ...
            var buildDataTableTimer = timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_Overall_TimerKey);
            ...
            {
                using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_DeleteExistingRecords_TimerKey))
                {
                ...
                await BulkInsertDataTable(
                    ...
                    timeout: 14400,
                    timerCollection?.GetStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_DtWriteExecution_TimerKey));
        ...
        protected DataTable BuildPerLocusPGroupDataTable(
            ...
            LongStopwatchCollection timerCollection = null)
        {
            var createDataTableObjectTimer = timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_CreateDtObject_TimerKey);
            var dataTable = new DataTable();
            ...
            foreach (var donor in donors)
            {
                donor.MatchingHla.GetLocus(locus).EachPosition((position, hlaAtLocusPosition) =>
                {
                    ...
                    using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_OutsideForeach_TimerKey))
                    {
                        foreach (var pGroup in hlaAtLocusPosition.MatchingPGroups)
                        {
                            using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_InsideForeach_TimerKey))
                            {
                                ...
                                var fetchPGroupIdsTimer = timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_FetchPGroupId_TimerKey);
                                var pGroupId = pGroupRepository.FindOrCreatePGroup(pGroup);
                                fetchPGroupIdsTimer?.Dispose();

                                using (timerCollection?.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_AddRowToDt_TimerKey))
                                {
                                    dataTable.Rows.Add(
                                        0,
                                        donor.DonorId,
