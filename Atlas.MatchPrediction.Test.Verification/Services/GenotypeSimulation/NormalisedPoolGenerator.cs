...
namespace Atlas.MatchPrediction.Test.Verification.Services.GenotypeSimulation
{
    ...
    internal class NormalisedPoolGenerator : INormalisedPoolGenerator
    {
        ...
        private async Task<NormalisedHaplotypePool> GenerateNormalisedPool(HaplotypeFrequenciesReaderResult sourceData)
        {
            ...
            return new NormalisedHaplotypePool(poolId, sourceData.HlaNomenclatureVersion, sourceData.TypingCategory, poolMembers);
        }

        private static decimal LowestFrequency(IEnumerable<FrequencyRecord> haplotypeFrequencies)
        {
        ...
        private static int CalculateCopyNumber(FrequencyRecord next, decimal lowestFrequency)
        {
