...
- At present, only data generated from the last validation run will be persisted to the associated Validation database (defined within the Functions app setting: `MatchPredictionValidation:Sql`)
  ...
  - Submitting match requests (without a patient ID to resume from) deletes existing requests and results, but patient and donor data is left intact.
...
2. Send match predictions requests, either by invoking the `SendMatchPredictionRequests` function or `ResumeMatchPredictionRequests` function.
  ...
  - Note: the maximum size of the topic subscription is 5Gb, and it is possible to hit that limit if you are submitting several million requests. Once the limit is reached, the match prediction request endpoint will return 4xx responses until the algorithm has consumed some of the requests. It is worth keeping an eye on the topic size, and stop/resuming requests sending accordingly.
