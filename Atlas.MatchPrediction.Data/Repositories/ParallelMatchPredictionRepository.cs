...
public interface IParallelMatchPredictionRepository
{
    ...
    /// <summary>
    /// Deletes batch rows belonging to runs that have status <see cref="ParallelMatchPredictionRunStatus.Abandoned"/>
    /// and whose <c>StatusDateUtc</c> (the time they were abandoned) is before <paramref name="cutoffUtc"/>, then marks
    /// those runs as <see cref="ParallelMatchPredictionRunStatus.AbandonedAndCleanedUp"/>. Transactional, mirroring
    /// <see cref="CleanupBatchesForRunsFinalisedBefore"/>. Parent run rows are intentionally retained.
    /// </summary>
    /// <returns>The number of batch rows deleted.</returns>
    Task<int> CleanupBatchesForRunsAbandonedBefore(DateTime cutoffUtc);
}
...
public class ParallelMatchPredictionRepository : IParallelMatchPredictionRepository
{
    ...
    public Task<AbandonedRunHeader> TryMarkRunAsAbandoned(int runId, DateTime nowUtc)
    {
        return context.ExecuteInTransactionAsync(async () =>
        {
            ...
            // FinalisationLeaseOwner is otherwise deliberately left null so a late-result replay can re-pick the run.
            var rowsUpdated = await context.ParallelMatchPredictionRuns
                .Where(r => r.Id == runId
                         && r.Status == ParallelMatchPredictionRunStatus.Running
                         && r.FinalisationLeaseOwner == null)
    ...
    public async Task<int> CleanupBatchesForRunsAbandonedBefore(DateTime cutoffUtc)
    {
        // Abandoned runs have no FinalisedTimeUtc, so StatusDateUtc (set when the run was abandoned) is the clock.
        var runIds = await context.ParallelMatchPredictionRuns
            .AsNoTracking()
            .Where(r => r.Status == ParallelMatchPredictionRunStatus.Abandoned
                     && r.StatusDateUtc < cutoffUtc
            )
            .Select(r => r.Id)
            .ToListAsync();
        ...
        return await context.ExecuteInTransactionAsync(async () =>
        {
            ...
            await context.ParallelMatchPredictionRuns
                .Where(r => runIds.Contains(r.Id))
                .ExecuteUpdateAsync(s => s
                    .SetProperty(r => r.Status, ParallelMatchPredictionRunStatus.AbandonedAndCleanedUp)
                    .SetProperty(r => r.StatusDateUtc, now)
                );
