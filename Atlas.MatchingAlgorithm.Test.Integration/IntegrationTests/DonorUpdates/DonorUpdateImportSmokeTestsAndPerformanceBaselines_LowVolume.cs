...
namespace Atlas.MatchingAlgorithm.Test.Integration.IntegrationTests.DonorUpdates
{
    ...
    public partial class DonorUpdateImportSmokeTestsAndPerformanceBaselines
    {
        ...
        public async Task ApplyDonorUpdatesToDatabase_ImportingAllDonorsFromExistingTestsAsSeparateBatches_ResultsInCorrectNumberOfDonorsAtEnd()
        {
            ...
            //New Invalid Donor.
            var donorInfo0B = new DonorInfoBuilder().WithHlaAtLocus(A, One, "*15:15").Build();
            ...
            var updatedDonor4 = new DonorInfoBuilder(donorInfo4.DonorId).WithDonorType(Cord).WithHlaAtLocus(A, One, "*15:15").Build();
            ...
            var updatedDonor5 = new DonorInfoBuilder(donorInfo5.DonorId).WithHlaAtLocus(A, One, "*15:15").Build();
        ...
        {
            var invalidNewHlaDonor = new DonorInfoBuilder().WithHlaAtLocus(A, One, "*15:15").Build().ToUpdate();
            yield return invalidNewHlaDonor;

            var existing = existingDonors.First().Donor;
            var invalidHlaDonorUpdate = new DonorInfoBuilder(existing.DonorId).WithHlaAtLocus(A, One, "*15:18").Build().ToUpdate();
