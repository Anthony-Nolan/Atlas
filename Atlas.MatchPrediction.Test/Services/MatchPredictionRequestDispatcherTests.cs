...
namespace Atlas.MatchPrediction.Test.Services
{
    ...
    {
        private static readonly Builder<SingleDonorMatchProbabilityInput> InvalidInput = SingleDonorMatchProbabilityInputBuilder.Default
        ...
        {
            dispatcher.Invoking(async service => await service.DispatchMatchPredictionRequestBatch(InvalidInput.Build(1)))
                .Should().NotThrow();
        }
        ...
        {
            var response = await dispatcher.DispatchMatchPredictionRequestBatch(InvalidInput.Build(1));
        ...
        public async Task DispatchMatchPredictionRequestBatch_MixedInput_OnlyReturnsMatchPredictionRequestIdForValidInput()
        {
            ...
            var response = await dispatcher.DispatchMatchPredictionRequestBatch(validInput.Concat(InvalidInput.Build(1)));
        ...
        {
            serviceBusClient.PublishToMatchPredictionRequestsTopic(default).ThrowsForAnyArgs(new Exception("error"));
