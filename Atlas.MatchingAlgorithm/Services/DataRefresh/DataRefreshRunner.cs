...
namespace Atlas.MatchingAlgorithm.Services.DataRefresh
{
    ...
    public class DataRefreshRunner : IDataRefreshRunner
    {
        ...
        private readonly IOrderedEnumerable<DataRefreshStage> orderedRefreshStages = EnumExtensions.EnumerateValues<DataRefreshStage>().OrderBy(x => x);
        ...
        /// Thus out data will look something like this:
        ///  F = Finished.
        ///  N = Not Finished.
        ///  S = Skip
        ///  C = Continue.
        ...
        private Dictionary<DataRefreshStage, DataRefreshStageExecutionMode> DetermineStageExecutionModes(DataRefreshRecord refreshRecord)
        {
            ...
            var previousStageWasCompletedInPreviousRun = false; //For the first stage there is no "previous stage".
            foreach (var stage in EnumExtensions.EnumerateValues<DataRefreshStage>().OrderBy(x=>x))
            {
                ...
                else if(previousStageWasCompletedInPreviousRun)
                {
                ...
                previousStageWasCompletedInPreviousRun = currentStageWasCompletedInPreviousRun;
            }
        ...
        private async Task ScaleDatabase(AzureDatabaseSize targetSize)
        {
            ...
            logger.SendTrace($"{LoggingPrefix}: Scaling database: {databaseName} to size {targetSize}");
