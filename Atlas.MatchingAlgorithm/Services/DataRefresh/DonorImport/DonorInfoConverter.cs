...
namespace Atlas.MatchingAlgorithm.Services.DataRefresh.DonorImport
{
        ...
        IDonorInfoConverter
    {
            ...
            string failureEventName)
        {
            ...
            // "is that the validation or the mapping?" - has an answer. (It was FluentValidation, until ATL-276
            // replaced it with SearchableDonorInformationFastValidator; these metrics are what showed that.)
            //
            ...
            var result = await ProcessBatchAsyncWithAnticipatedExceptions<ValidationException>(
                ...
                info =>
                {
                    ...
                    try
                    {
                        // Deliberately not SearchableDonorInformationValidator: identical outcome, but this runs once per donor in the
                        // registry, and building a FluentValidation rule tree per call dominated the cost of the entire conversion.
                        // See the remarks on SearchableDonorInformationFastValidator.
                        SearchableDonorInformationFastValidator.ValidateAndThrow(info);
                    }
                    finally
                    {
