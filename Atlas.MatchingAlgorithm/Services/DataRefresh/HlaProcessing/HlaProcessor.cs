...
namespace Atlas.MatchingAlgorithm.Services.DataRefresh.HlaProcessing
{
    ...
    public class HlaProcessor : IHlaProcessor
    {
        ...
        private async Task PerformHlaUpdate(string hlaNomenclatureVersion, int refreshRecordId, int? lastProcessedDonor, bool continueExistingProcessing)
        {
            ...
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.BatchProgress_TimerKey, "Hla Batch Overall Processing. Inner Operation is UpdateDonorBatch", null, progressReports)) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaExpansion_TimerKey, " * Hla Expansion, during HlaProcessing")) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.NewPGroupInsertion_Overall_TimerKey, " * Ensuring all PGroups exist in the DB, during HlaProcessing (no actual DB writing, just processing)")) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.NewPGroupInsertion_Flattening_TimerKey, " * * Flatten the donors' PGroups, during EnsureAllPGroupsExist, during HlaProcessing")) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.NewPGroupInsertion_FindNew_TimerKey, " * * Check PGroups against known dictionary, during EnsureAllPGroupsExist, during HlaProcessing"))
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_Overall_TimerKey, " * UpsertMatchingPGroupsAtSpecifiedLoci, during HlaProcessing")) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_Overall_TimerKey, " * * Time setting up Hla BulkInsert statements, during HlaProcessing")) 
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_Overall_TimerKey, " * * * Data Table Build, in Hla BulkInsert SETUP, during HlaProcessing"))
            using (timerCollection.InitialiseDisabledStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_CreateDtObject_TimerKey, " * * * * Creating blank DataTable object, in DataTableBuild, in Hla BulkInsert SETUP, during HlaProcessing"))
            using (timerCollection.InitialiseDisabledStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_OutsideForeach_TimerKey, " * * * * Outside the innermost foreach of method, in DataTableBuild, in Hla BulkInsert SETUP, during HlaProcessing"))
            using (timerCollection.InitialiseDisabledStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_InsideForeach_TimerKey, " * * * * Inside the innermost foreach of method, in DataTableBuild, in Hla BulkInsert SETUP, during HlaProcessing"))
            using (timerCollection.InitialiseDisabledStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_FetchPGroupId_TimerKey, " * * * * Fetch PGroup Id, in DataTableBuild, in Hla BulkInsert SETUP, during HlaProcessing") )
            using (timerCollection.InitialiseDisabledStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_BuildDataTable_AddRowToDt_TimerKey, " * * * * Raw DataTable Row Add, in DataTableBuild, in Hla BulkInsert SETUP, during HlaProcessing") )
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BulkInsertSetup_DeleteExistingRecords_TimerKey, " * * * Delete Existing records, in Hla BulkInsert SETUP, during HlaProcessing") )
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_BlockingWait_TimerKey, " * * Time spent in `Task.WhenAll`, JUST waiting on HlaInsert tasks to Complete, during HlaProcessing") )
            using (timerCollection.InitialiseStopwatch(StopwatchKeys.HlaProcessor.HlaUpsert_DtWriteExecution_TimerKey, " * * * Total Time spent across all threads, writing BulkInserts during HlaInsert operation, during HlaProcessing", null, summaryReportWithThreadingCount))
            {
                ...
                foreach (var donorBatch in batchedDonors)
                {
                    ...
                    using (timerCollection.TimeInnerOperation(StopwatchKeys.HlaProcessor.BatchProgress_TimerKey))
                    {
            ...
            LongStopwatchCollection timerCollection)
        {
            ...
            var timedInnerOperation = timerCollection.TimeInnerOperation(StopwatchKeys.HlaProcessor.HlaExpansion_TimerKey);
            ...
            using (timerCollection.TimeInnerOperation(StopwatchKeys.HlaProcessor.NewPGroupInsertion_Overall_TimerKey))
            {
        ...
        {
            var flatteningTimer = timerCollection.TimeInnerOperation(StopwatchKeys.HlaProcessor.NewPGroupInsertion_Flattening_TimerKey);
