...
namespace Atlas.MatchingAlgorithm.Services.Search.Matching
{
    ...
    internal class DonorMatchingService : IDonorMatchingService
    {
            ...
            var resultStream = orderedLoci
                .Aggregate(
                    ...
                    (aggregator, locus) =>
                    {
                        ...
                        return (
                            MatchAtLocus(criteria, locus, matchedLoci, cutOffDate, partialResultStream),
            ...
            using (searchLogger.RunTimed($"Matching at Locus {locus}. (Timing cumulative across loci.)", verboseAtStart: true))
            {
                ...
                if (!CanFilterByDonorIds(criteria, matchedLoci))
                {
                    var results = previousLociResultStream == null
                        ...
                        : await LoadAllResultsFromPreviousLociStream(previousLociResultStream);
        ...
        private async Task<Dictionary<int, MatchResult>> LoadAllResultsFromPreviousLociStream(IAsyncEnumerable<MatchResult> previousLociResultStream)
        {
            ...
            var results = (await System.Linq.AsyncEnumerable.ToListAsync(previousLociResultStream)).ToDictionary(x => x.DonorId, x => x);
