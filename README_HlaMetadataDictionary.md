...
So whenever `RecreateHlaMetadataDictionary` actually rewrites storage - on the forced, specific-version route and on the normal, version-changing route alike - it stamps a row for that version in the `HlaMetadataDictionaryRecreations` table: a fresh GUID, plus the time. The stamp carries no meaning beyond being different from the last one. The recreating process then drops its own cached copy at once, without waiting to notice its own stamp.

Every app holding a cached copy runs `HlaMetadataDictionaryRecreationWatcher`, a `BackgroundService` registered by `RegisterHlaMetadataDictionaryCacheInvalidation()`. It reads the stamps once a minute (`HlaMetadataDictionary:CacheInvalidationPollIntervalSeconds`), and calls `IHlaMetadataCacheInvalidator` for any version whose stamp has changed since its last look.

Four things to know about this:
* **Polling, rather than a message, because every instance has to find out.** A Service Bus subscription is a competing-consumer queue: one shared by an app's instances would hand each notification to exactly one of them, leaving every other instance serving its stale copy - and the Matching Algorithm app scales out. Giving each instance a subscription of its own is not possible through a `ServiceBusTrigger`, because **the Functions host resolves `%...%` binding expressions from application settings and never sees configuration added inside the worker** ([documented platform behaviour](https://learn.microsoft.com/azure/azure-functions/dotnet-isolated-process-guide#start-up-and-configuration)) - so a per-instance subscription name computed at worker start-up cannot be bound to. Every instance reading the same stamp sidesteps all of it: nothing is created per instance, nothing needs cleaning up when one is scaled away, and a new instance starts correct because it starts empty. A `ServiceBusProcessor` in a hosted service would also have worked, and was rejected for the operational cost - Manage rights on the topic, entities created and deleted at runtime, and a start-up dependency on Service Bus.
* **The cost is latency, measured against 24 hours rather than zero.** A recreation is noticed within one poll interval. After running `RefreshHlaMetadataDictionaryToSpecificVersion`, allow about a minute before re-testing.
* **No new infrastructure and no new credentials.** The stamp table lives in the storage account the dictionary already uses, reached over the connection it already has, and is created on first write. Every app that can recreate the dictionary stamps it automatically - including `Atlas.MatchingAlgorithm.Api` - with nothing to opt into.
* **Nothing fails because of this.** A stamp that cannot be written is logged as an error and the recreation still succeeds: storage is already rewritten by that point, and failing would fail a data refresh whose first stage had in fact succeeded. A poll that cannot read the table is logged as a warning and retried on the next tick, leaving the cache exactly where it would have been anyway.
...
* `IHlaMetadataCacheInvalidator`
  ...
  * Called by `HlaMetadataDictionaryRecreationWatcher` when it notices a recreation, and directly by the process that did the recreating. See [Cache Invalidation](#cache-invalidation) above.
