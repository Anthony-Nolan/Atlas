...
namespace Atlas.MatchPrediction.Services.ExpandAmbiguousPhenotype
{
    internal interface IAmbiguousPhenotypeExpander
    {
        ...
        public ISet<PhenotypeInfo<string>> ExpandPhenotype(
            PhenotypeInfo<IReadOnlyCollection<string>> allelesPerLocus,
            IReadOnlyCollection<LociInfo<string>> allowedHaplotypes);
    }
    ...
    internal class AmbiguousPhenotypeExpander : IAmbiguousPhenotypeExpander
    {
            ...
            IReadOnlyCollection<LociInfo<string>> allowedHaplotypes)
        {
            var genotypes = (from a1 in allelesPerLocus.A.Position1
                ...
                from drb1 in allelesPerLocus.Drb1.Position1
                from drb2 in allelesPerLocus.Drb1.Position2
                where allowedHaplotypes.Any(h => h.A == a1)
                where allowedHaplotypes.Any(h => h.A == a2)
                where allowedHaplotypes.Any(h => h.B == b1)
                where allowedHaplotypes.Any(h => h.B == b2)
                where allowedHaplotypes.Any(h => h.C == c1)
                where allowedHaplotypes.Any(h => h.C == c2)
                where allowedHaplotypes.Any(h => h.Dqb1 == dqb1)
                where allowedHaplotypes.Any(h => h.Dqb1 == dqb2)
                where allowedHaplotypes.Any(h => h.Drb1 == drb1)
                where allowedHaplotypes.Any(h => h.Drb1 == drb2)
                select new PhenotypeInfo<string>
                {
                    A = new LocusInfo<string> {Position1 = a1, Position2 = a2},
                    B = new LocusInfo<string> {Position1 = b1, Position2 = b2},
