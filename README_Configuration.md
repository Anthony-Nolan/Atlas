...
| ELASTIC-SERVICE-PLAN-FOR-PUBLIC-API | N/A | N/A | Determines whether a separate elastic plan should be created that only hosts the `<env>-ATLAS-API` app (`true`), or whether the API app should run on the same elastic plan as all other function apps (`false`). Default is `true`.<br><br>**Important Release Notes**:<br>* Changing the value of this var will cause the `<env>-ATLAS-API` functions app to be recreated and its **host keys to be regenerated**. <br>* The `WEBSITE-RUN-FROM-PACKAGE` var must be set to `0` within the same release to allow host key generation; it can be reset to `1` in subsequent releases.<br>* On first release of this var (added in [Atlas v2.0.3](/Atlas.Functions.PublicApi/CHANGELOG_Atlas.md#203)) the default value of `true` should be used. The var can then be set to `false` on subsequent releases, where you wish the `<env>-ATLAS-API` app to run on the same plan as all other function apps. |
