...
namespace Atlas.DonorImport.Data.Repositories
{
    ...
    public class DonorImportLogRepository : IDonorImportLogRepository
    {
        ...
        public async Task DeleteDonorLogBatch(IReadOnlyCollection<string> externalDonorCodes)
        {
            await using (var connection = new SqlConnection(ConnectionString))
            {
                var sql = $"DELETE FROM {DonorLogTableName} WHERE {ExternalDonorCodeColumnName} IN @atlasIds";
                connection.Open();

                foreach (var atlasIds in externalDonorCodes.Batch(DonorImportBatchSize))
                {
                    await connection.ExecuteAsync(sql, new { atlasIds });
                }
