...
- Set up sensible initial data.
  ...
  - Importing Haplotype Frequency Sets.
    ...
    - Edit the `<gitRoot>/MiscTestingAndDebuggingResources/MatchPrediction/initial-hf-set.json` file so `nomenclatureVersion` is set to the nomenclature version that was used in the data refresh.
      - SQL Server The nomenclature version that was used in the data refresh  can be found in SQL Server - `AtlasMatchingPersistent` -> `dbo.DataRefreshHistory` and then the `HlaNomenclatureVersion` for the active database.
...
- Run a search (avoiding MAC lookups).
  ...
  - Then hit the `Search` endpoint within `Atlas.Functions.PublicApi` with the content of `<gitRoot>\MiscTestingAndDebuggingResources\MatchingAlgorithm\ZeroResultsSearch.json` as the requests body.
