...
namespace Atlas.Common.GeneticData.Hla.Services
{
    ...
    internal class HlaCategorisationService : IHlaCategorisationService
    {
        ...
        {
            {new Regex($"^{MolecularFirstFieldPattern}:(?!XX$)[A-Z]{{2,}}$"), HlaTypingCategory.NmdpCode},
            {new Regex($"^{MolecularFirstFieldPattern}:XX$"), HlaTypingCategory.XxCode},
            {new Regex($"^{MolecularFirstFieldPattern}(:{SingleFieldPattern}){{2}}G$"), HlaTypingCategory.GGroup},
            {new Regex($"^{MolecularFirstFieldPattern}:{SingleFieldPattern}P$"), HlaTypingCategory.PGroup},
            {new Regex($"^(?!0){SingleFieldPattern}$"), HlaTypingCategory.Serology},
            {new Regex($"^{AlleleDesignationPattern}$"), HlaTypingCategory.Allele},
            {new Regex($"^{AlleleDesignationPattern}(\\/{AlleleDesignationPattern}){{1,}}$"), HlaTypingCategory.AlleleStringOfNames},
            {new Regex($"^{MolecularFirstFieldPattern}:{AlleleFinalFieldPattern}(\\/{AlleleFinalFieldPattern}){{1,}}$"), HlaTypingCategory.AlleleStringOfSubtypes}
        };
