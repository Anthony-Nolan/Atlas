...
namespace Atlas.Common.Maths
{
    ...
    public static class Combinations
    {
        ...
        /// <param name="m_combinationSize"></param>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        private static IEnumerable<IReadOnlyList<T>> AllCombinations<T>(IReadOnlyList<T> collection, int m_combinationSize)
        {
            var allIndexCombinations = AllCombinations(collection.Count, m_combinationSize).ToList();
        ...
        private static IEnumerable<int[]> AllCombinations(int n_sourceCollectionSize, int m_combinationSize)
        {
            var result = new int[m_combinationSize];
            var stack = new Stack<int>(m_combinationSize);
            stack.Push(0);
            while (stack.Count > 0)
            {
