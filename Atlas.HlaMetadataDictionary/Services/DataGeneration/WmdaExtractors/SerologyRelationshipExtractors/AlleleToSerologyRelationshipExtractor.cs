...
namespace Atlas.HlaMetadataDictionary.Services.DataGeneration.WmdaExtractors.SerologyRelationshipExtractors
{
    internal class AlleleToSerologyRelationshipExtractor : WmdaDataExtractor<RelDnaSer>
    {
        ...
        private readonly Regex regex = new Regex(@"^(\w+\*)\;([\w:]+)\;([\d\/\\?]*);([\d\/\\?]*)\;([\d\/\\?]*)\;([\d\/\\?]*)(?:\;([\d\/\\?]*))?$", RegexOptions.Compiled);

        public AlleleToSerologyRelationshipExtractor() : base(FileName)
        {
        }

        protected override RelDnaSer MapLineOfFileContentsToWmdaHlaTyping(string line)
        {
