...
namespace Atlas.HlaMetadataDictionary.InternalModels.MetadataTableRows
{
    internal class HlaMetadataTableRow : TableEntity
    {
        ...
        public override IDictionary<string, EntityProperty> WriteEntity(OperationContext operationContext)
        {
            ...
            var batchedSerialisedInfo = SerialisedHlaInfo.Batch(MaximumRowSize).ToList();
            var numberOfBatches = batchedSerialisedInfo.Count;
            ...
            // These additional properties are combined again on read, so any consumers of this class in code still 
            for (var i = 0; i < numberOfBatches; i++)
            {
                var batch = batchedSerialisedInfo[i].Aggregate("", (x, y) => x + y);
                var batchPropertyName = i == 0 ? nameof(SerialisedHlaInfo) : nameof(SerialisedHlaInfo) + i;
                properties.Add(batchPropertyName, new EntityProperty(batch));
            }
