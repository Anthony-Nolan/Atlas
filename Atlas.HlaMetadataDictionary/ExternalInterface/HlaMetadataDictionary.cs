...
namespace Atlas.HlaMetadataDictionary.ExternalInterface
{
    ...
    internal class HlaMetadataDictionary : IHlaMetadataDictionary
    {
        ...
        public async Task<string> RecreateHlaMetadataDictionary(CreationBehaviour creationConfig)
        {
            ...
            {
                logger.SendTrace($"HLA-METADATA-DICTIONARY REFRESH: Recreating HLA Metadata dictionary from latest WMDA database version.",
                    LogLevel.Info);
            ...
            {
                logger.SendTrace(
                    $"HLA-METADATA-DICTIONARY REFRESH: HLA Metadata dictionary was already up to date with latest WMDA nomenclature version, so did not update.",
                    LogLevel.Info);
            }
