...
| 2. Check the `donor-import-results` topic on `<env>-atlas` service bus | There should be a success donor import message with next properties:<br> - FileName,<br> - WasSuccessful = true,<br> - ImportedDonorCount  = total number of donors in the test file,<br> - FailedDonorCount = 0 |
...
| 4. Run matching algorithm function debug donor endpoint (see **test data** section): in request body, submit `int[]` of Atlas donor ids taken from response of step 3. | - Response code: 200 <br>- `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1.<br> - Donors metadata and HLA data in response equals to donors metadata and HLA data in Donor import file used in step 1. <br> - In `presentDonors` object, every donor should have `IsAvailableForSearch` set to `true`. |
...
| Test step | Expected Result |
| --------- | --------------- |
| 1. Generate a Donor Import .json file with a few donors. You can use donor import file content example from Test Data section, but you should change `"recordId"`:<br> - Select one or few donors in `<env>atlas` DB, `Donors.Donors` table.<br> - Use `ExternalDonorCode` values as `"recordId"` in import file. | Valid donor import file.|
| 2. Run donor import function debug donor endpoint (see **test data** section): in request body, submit `string[]` of donor record ids taken from the `recordId` fields within the Donor import file. Note the Atlas donor IDs returned within the response.| - Response code: 200 <br> - `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file from step 1.|
| 3. Upload Donor Import .json file to `donors` folder of `<env>atlasstorage` | Donor Import file successfully uploaded. |
...
| 5. Run donor import function debug donor endpoint (see **test data** section): in request body, submit `string[]` of donor record ids taken from the `recordId` fields within the Donor import file from step 1. | - Response code: 200 <br> - `"present"` value in `"donorCounts"` object of response equals to `0`.<br> - `"absent"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1.|
| 6. Run matching algorithm function debug donor endpoint (see **test data** section): in request body, submit `int[]` of Atlas donor ids taken from response of step 2. | - Response code: 200 <br>- `"present"` value in `"donorCounts"` object should equal number of submitted ids. <br> - In `presentDonors` object, every donor should have `IsAvailableForSearch` set to `false`. |
---
...
| 4. Run donor import function debug donor endpoint (see **test data** section): in request body, submit `string[]` of donor record ids taken from the `recordId` fields within the Donor import file from step 1. | - Response code: 200 <br> - `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1.<br> - Donors metadata and HLA data in response equals to donors metadata and HLA data in Donor import file used in step 2. |
| 5. Run matching algorithm function debug donor endpoint (see **test data** section): in request body, submit `int[]` of Atlas donor ids taken from response of step 4. | - Response code: 200 <br>- `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1.<br> - Donors metadata and HLA data in response equals to donors metadata and HLA data in Donor import file used in step 1.<br> - In `presentDonors` object, every donor should have `IsAvailableForSearch` set to `true`. |
...
| 5. Go to Atlas Application Insights Logs ("`<ENV>`-ATLAS") within Azure web portal and check donor import logs by next query:<br>`traces`<br>`\| where message contains "Failed to import"`| There are logs in `<ENV>-ATLAS` where you could find info about failed donors|
| 6. Run donor import function debug donor endpoint (see **test data** section): in request body, submit `string[]` of donor record ids taken from the `recordId` fields within the Donor import file from step 1. | - Response code: 200 <br> - `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1. |
| 7. Run matching algorithm function debug donor endpoint (see **test data** section): in request body, submit `int[]` of Atlas donor ids taken from response of step 6. | - Response code: 200 <br>- `"present"` value in `"donorCounts"` object of response equals to quantity of donors in Donor import file used in step 1. |
| 8. [**Clean Up**] Delete imported donors from DB.<br>Upload donor import file in deletion mode (example in test data section) with `recordIds` uploaded in step 1|Donors uploaded in step 1 deleted from DB|
---
<br>
