...
namespace Atlas.Common.Test.Hla.Services
{
    ...
    public class AlleleStringSplitterServiceTests
    {
        ...
        {
            splitterService.Invoking(service => service.GetAlleleNamesFromAlleleString(hlaName)).Should().Throw<Exception>();
        }

        [Test]
        public void GetAlleleNamesFromAlleleString_InvalidHla_RaiseError()
        {
            splitterService.Invoking(service => service.GetAlleleNamesFromAlleleString("NOT-VALID-HLA")).Should().Throw<Exception>();
        }
    }
}
