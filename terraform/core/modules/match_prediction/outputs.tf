output "function_app" {
  value = {
    hostname = azurerm_windows_function_app.atlas_match_prediction_function.default_hostname
    app_name = local.atlas_match_prediction_function_name
    id       = azurerm_windows_function_app.atlas_match_prediction_function.id
  }
}
