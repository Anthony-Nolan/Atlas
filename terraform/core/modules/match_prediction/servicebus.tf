...
// --- Parallel ACA Worker MPA path ---
// `parallel-match-prediction-requests` carries batch-blob locations from the orchestrator to the ACA Worker.
// `parallel-match-prediction-results` is session-enabled (RequiresSession=true) so the aggregator holds an exclusive
//  lock per SearchRequestId and can safely accumulate results without race conditions.

resource "azurerm_servicebus_topic" "parallel-match-prediction-requests" {
  name                  = "parallel-match-prediction-requests"
  namespace_id          = var.servicebus_namespace.id
  auto_delete_on_idle   = var.default_servicebus_settings.long-expiry
  default_message_ttl   = var.default_servicebus_settings.long-expiry
  max_size_in_megabytes = var.default_servicebus_settings.default-bus-size
...
resource "azurerm_servicebus_subscription" "parallel-match-prediction-request-runner" {
  name                                 = "match-prediction"
...
resource "azurerm_servicebus_subscription" "parallel-match-prediction-results-aggregator" {
  name                                 = "aggregator"
